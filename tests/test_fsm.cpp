...
TEST(FileSystemManagerTest, ReadWriteFile) {
    FileSystemManager* fsm = new FileSystemManager();
    fsm->mount("test_fs_readwrite.img");
    fsm->add_node(0, "test_file.txt", false, 0644);
    Flouds* flouds = fsm->get_flouds();
    size_t node_id = flouds->child(0, 0);

    const char* data = "Hello, World!";
    fsm->write_file(node_id, data, strlen(data), 0);

    char buffer[20];
    fsm->read_file(node_id, buffer, sizeof(buffer), 0);

    EXPECT_STREQ(buffer, data);

    delete fsm;
    std::remove("test_fs_readwrite.img");
}